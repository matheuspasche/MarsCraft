...
#' @importFrom tidyr unnest
#' @export
trata_bdc_processes <- function(initial_path) {
  ...
  if ("Lawsuits_Lawsuits_Parties" %in% colnames(data)) {
    data <- data %>%
...
#' @importFrom glue glue
trata_bdc_owners_lawsuits <- function(initial_path) {
  ...
  if (
    nrow(data) == 1 && data$OwnersLawsuits_TotalLawsuits != 0 ||
    all(data$OwnersLawsuits_TotalLawsuits) != 0
  ) {
    data <- data %>%
...
#'
#' @param initial_path O caminho para o arquivo inicial.
#' @return Nenhum valor é retornado explicitamente, o arquivo data é salvo no diretório de destino.
#' @importFrom readr read_rds
#' @importFrom dplyr %>%
#' @importFrom tidyr unnest
#' @importFrom tidyr pivot_longer
...
trata_bdc <- function(initial_path, type) {
  ...
  if(type == 'owners_lawsuits'){
    trata_bdc_owners_lawsuits(initial_path= initial_path)
